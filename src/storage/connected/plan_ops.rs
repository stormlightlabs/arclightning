...
use crate::plan::{PlanDocument, PlanEntries, PlanTaskEntry};
...
fn resolve_plan_task_reference(
    ...
    if let Ok(id) = TaskId::parse(reference) {
        ...
        return match record {
            ...
            None => Err(StorageError::InvalidPlanInput(
                crate::plan::PlanError::UnknownDependency {
    ...
    if matches.len() > 1 {
        return Err(StorageError::InvalidPlanInput(
            crate::plan::PlanError::AmbiguousDependency {
                task: task_path.to_owned(),
                dependency: reference.to_owned(),
            },
        ));
    ...
    Err(StorageError::InvalidPlanInput(
        crate::plan::PlanError::UnknownDependency { task: task_path.to_owned(), dependency: reference.to_owned() },
    ))
}
