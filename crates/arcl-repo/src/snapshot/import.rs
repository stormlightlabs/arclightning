...
use arcl_core::domain::{EpicId, TaskDependency, TaskId};
use arcl_store::{Database, Graph, SnapshotBaseFile, StorageError};
...
fn validate_relationships(
    ...
    for idea in &graph.ideas {
        match (idea.status, idea.promoted_to) {
            (arcl_core::domain::IdeaStatus::Promoted, Some(epic_id)) => {
            ...
            (arcl_core::domain::IdeaStatus::Promoted, None) => {
                return Err(field_error(
                    source_for(sources, idea.id),
                    "status",
    ...
    for epic in &graph.epics {
        if let Some(idea_id) = epic.source_idea {
            ...
            if idea.status != arcl_core::domain::IdeaStatus::Promoted || idea.promoted_to != Some(epic.id) {
                return Err(relationship_error(
                    source_for(sources, epic.id),
    ...
    let mut task_keys = HashMap::<(arcl_core::domain::MilestoneId, String), PathBuf>::new();
...
fn idea_from_record(record: &IdeaRecord) -> arcl_core::domain::Idea {
    arcl_core::domain::Idea {
        id: record.id,
...
fn release_from_record(record: &ReleaseRecord) -> arcl_core::domain::Release {
    arcl_core::domain::Release {
        id: record.id,
        title: record.title.clone(),
        description: record.description.clone(),
        status: record.status,
...
fn epic_from_record(record: &EpicRecord) -> arcl_core::domain::Epic {
    arcl_core::domain::Epic {
        id: record.id,
...
fn milestone_from_record(record: &MilestoneRecord) -> arcl_core::domain::Milestone {
    arcl_core::domain::Milestone {
        id: record.id,
        epic_id: record.epic,
        title: record.title.clone(),
        description: record.description.clone(),
...
fn task_from_record(record: &TaskRecord) -> arcl_core::domain::Task {
    arcl_core::domain::Task {
        id: record.id,
...
mod tests {
    use super::*;
    use arcl_core::domain::{ContainerStatus, EpicId, IdeaId, IdeaStatus, MilestoneId, TaskPriority, TaskStatus};

    use crate::snapshot::SnapshotExportError;
    use std::fs;
    ...
    fn complete_graph() -> Graph {
        ...
        Graph {
            ideas: vec![arcl_core::domain::Idea {
                id: idea_id,
                title: "Idea".to_owned(),
            ...
            releases: Vec::new(),
            epics: vec![arcl_core::domain::Epic {
                id: epic_id,
                release_id: None,
                title: "Epic".to_owned(),
                description: String::new(),
            ...
            milestones: vec![arcl_core::domain::Milestone {
                id: milestone_id,
                epic_id,
            ...
            tasks: vec![arcl_core::domain::Task {
                id: task_id,
                milestone_id,
                parent_id: None,
                title: "Task".to_owned(),
    ...
    fn normalized_spec_paths_must_be_unique() {
        ...
        graph.epics.push(arcl_core::domain::Epic {
