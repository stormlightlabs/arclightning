...
use arcl_core::domain::{self, DomainError, MilestoneId, Task, TaskAction, TaskId, TaskPriority, TaskStatus};
...
pub fn handoff(connection: &mut Connection, id: TaskId, note: String) -> Result<Task> {
    let transaction = connection.transaction()?;
    let current = read_one(&transaction, &id)?.ok_or_else(|| StorageError::TaskNotFound { id: id.to_string() })?;
    if current.status != arcl_core::domain::TaskStatus::InProgress {
    ...
    transaction.commit()?;
    Ok(Task { status: arcl_core::domain::TaskStatus::Parked, handoff: note, ..current })
}
...
#[cfg(test)]
mod tests {
    use crate::storage::{Database, StorageError, TaskUpdate};
    use arcl_core::domain::{self, DomainError, TaskAction, TaskPriority, TaskStatus};
