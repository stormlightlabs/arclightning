...
use arcl_core::domain::{DomainError, Idea, IdeaAction, IdeaId, IdeaStatus, validate_title};
...
fn decode_idea(
    ...
    idea.promoted_to = promoted_to
        .map(|id| arcl_core::domain::EpicId::parse(&id).map_err(DomainError::from))
        .transpose()?;
    Ok(idea)
}
...
mod tests {
    ...
    use arcl_core::domain::{DomainError, IdeaStatus};
