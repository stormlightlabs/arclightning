...
pub struct ListFilter {
    ...
    pub spec_id: Option<arcl_core::domain::SpecId>,
    /// Connected-model plan filter used by the CLI adapter.
    pub plan_id: Option<arcl_core::domain::PlanId>,
    /// Connected-model phase filter used by the CLI adapter.
    pub phase_id: Option<arcl_core::domain::PhaseId>,
}
...
impl Graph {
    fn show(&self, id: &str) -> Result<ShowView> {
        if id.starts_with(IdeaId::PREFIX) {
            let id = IdeaId::parse(id)
                .map_err(|error| StorageError::InvalidTask(arcl_core::domain::DomainError::InvalidId(error)))?;
            let record = self
                .ideas
                .iter()
        ...
        Err(StorageError::InvalidTask(arcl_core::domain::DomainError::InvalidId(
            arcl_core::domain::IdError::InvalidPrefix {
                entity: "record",
                expected: "arcl-<kind>-",
                value: id.to_owned(),
            },
    ...
    pub fn tree(&self, root: Option<&str>) -> Result<Vec<TreeNode>> {
        match root {
            ...
            Some(id) => Err(StorageError::InvalidTask(arcl_core::domain::DomainError::InvalidId(
                arcl_core::domain::IdError::InvalidPrefix {
                    entity: "record",
                    expected: "arcl-<kind>-",
                    value: id.to_owned(),
    ...
    fn task_node(&self, task: &Task, path: &mut HashSet<TaskId>) -> Result<TreeNode> {
        if !path.insert(task.id) {
            return Err(StorageError::InvalidTask(arcl_core::domain::DomainError::ParentCycle {
...
impl DomainErrorToStorage {
    fn into_storage(error: arcl_core::domain::IdError) -> StorageError {
        StorageError::InvalidTask(arcl_core::domain::DomainError::InvalidId(error))
    }
}
