...
use arcl_core::domain::{ContainerAction, ContainerStatus, DomainError, EpicId, Milestone, MilestoneId};
...
pub fn update(
    ...
    if let Some(title) = &title {
        arcl_core::domain::validate_title(title).map_err(StorageError::InvalidMilestone)?;
    }
    if let Some(position) = position {
        arcl_core::domain::validate_position(position).map_err(StorageError::InvalidMilestone)?;
    }
...
mod tests {
    ...
    use arcl_core::domain::{DomainError, MilestoneId};
    ...
    fn milestone_mutations_validate_ownership_and_order_ties_by_id() {
        ...
        let error = database
            .create_milestone(arcl_core::domain::EpicId::new(), "Missing".to_owned(), String::new(), 0)
