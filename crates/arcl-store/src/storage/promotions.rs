...
use arcl_core::domain::{ContainerStatus, Epic, EpicId, IdeaId, IdeaStatus, ReleaseId};
...
pub struct Promotion {
    ...
    pub idea: arcl_core::domain::Idea,
    ...
    match (idea.status, relationship) {
        (IdeaStatus::Promoted, Some(epic_id)) => {
            let epic_id = EpicId::parse(&epic_id)
                .map_err(arcl_core::domain::DomainError::from)
        ...
        (IdeaStatus::Captured, None) => {
            let epic = arcl_core::domain::Epic::new(title, description, spec_path, release_id)
                .map_err(StorageError::InvalidEpic)?;
...
mod tests {
    ...
    use arcl_core::domain::IdeaStatus;
    ...
    fn discarded_and_invalid_release_promotions_leave_the_idea_unchanged() {
        ...
        let captured = database
            ...
            .expect("idea creates");
        let missing = arcl_core::domain::ReleaseId::new();
        assert!(matches!(
