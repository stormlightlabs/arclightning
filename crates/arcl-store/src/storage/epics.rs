...
use arcl_core::domain::{ContainerAction, ContainerStatus, DomainError, Epic, EpicId, ReleaseId};
...
pub fn update(
    ...
    if let Some(title) = &title {
        arcl_core::domain::validate_title(title).map_err(StorageError::InvalidEpic)?;
    }
...
fn decode_epic((id, release_id, title, description, spec_path, status, source_idea): RawEpic) -> Result<Epic> {
    ...
    epic.source_idea = source_idea
        .map(|id| arcl_core::domain::IdeaId::parse(&id).map_err(DomainError::from))
        .transpose()
        .map_err(StorageError::InvalidEpic)?;
    Ok(epic)
}
