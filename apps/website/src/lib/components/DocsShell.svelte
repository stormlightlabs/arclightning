...
<div class="docs-layout">
  ...
  <main id="main-content" class="docs-main">
    <nav class="breadcrumbs" aria-label="Breadcrumbs" data-pagefind-ignore><a href="/">Home</a><span aria-hidden="true">/</span><span>{doc.section}</span><span aria-hidden="true">/</span><span aria-current="page">{doc.title}</span></nav>
    <article data-pagefind-body>
      <header class="doc-heading"><p class="eyebrow">{doc.section}</p><h1>{doc.title}</h1><div class="doc-heading__secondary"><CopyMarkdown markdown={doc.markdown} slug={doc.slug} /><p>{doc.description}</p></div></header>
